...
var (
	// TODO(maruel): There will be OS specific flags. For example a user may (or
	// may not) want to enable I²C bus #0 on a Raspberry Pi.
	i2c    = flag.Bool("-i2c", true, "enable I²C")
	spi    = flag.Bool("-spi", true, "enable SPI")
	force  = flag.Bool("-force", false, "bypass safeguards")
...
func setupArmbian() error {
	...
	// TODO(maruel): This is not really a good rule.
	// TODO(maruel): Use plugdev instead of dialout but only on debian derivatives?
	content := `# This file was generated by pio-setup.
SUBSYSTEM=="gpio*", PROGRAM="/bin/sh -c 'chown -R root:dialout /sys/class/gpio && chmod -R 770 /sys/class/gpio;chown -R root:dialout /sys$devpath && chmod -R 770 /sys$devpath'"
`
	return setupUdev(content)
}
