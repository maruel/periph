...
// Package ssd1327 controls a 128x64 4 bits OLED display via a SSD1327
// controller.
//
...
//
// The SSD1327 is a write-only device. It can be driven on either I²C or SPI
...
//
// Product page:
// http://www.solomon-systech.com/en/product/advanced-display/oled-display-driver-ic/ssd1327/
//
...
//
// Values are different than ssd1306.
const (
	FrameRate2   FrameRate = 7
	FrameRate3   FrameRate = 4
	FrameRate4   FrameRate = 5
	FrameRate5   FrameRate = 6
	FrameRate6   FrameRate = 0
	FrameRate32  FrameRate = 1
	FrameRate64  FrameRate = 2
...
// NewSPI returns a Dev object that communicates over SPI to a SSD1327 display
// controller.
//
// If rotated is true, turns the display by 180°
//
// The SSD1327 can operate at up to 3.3Mhz, which is much higher than I²C. This
...
// be reinstantiated.
func NewSPI(p spi.Port, dc gpio.PinOut, o *Opts) (*Dev, error) {
	if dc == gpio.INVALID {
		return nil, errors.New("ssd1327: use nil for dc to use 3-wire mode, do not use gpio.INVALID")
	}
	bits := 8
	if dc == nil {
	...
	return newDev(c, o, true, dc)
}

// NewI2C returns a Dev object that communicates over I²C to a SSD1327 display
// controller.
//
// If rotated, turns the display by 180°
func NewI2C(i i2c.Bus, o *Opts) (*Dev, error) {
	// Maximum clock speed is 1/2.5µs = 400KHz.
	return newDev(&i2c.Dev{Bus: i, Addr: 0x3C}, o, false, nil)
}
...
type Dev struct {
	...
	// Display size controlled by the SSD1327.
...
//
// It is a 4 bits color model, as implemented by image4bit.Gray4.
...
func (d *Dev) Draw(r image.Rectangle, src image.Image, sp image.Point) {
	var next []byte
	if img, ok := src.(*image4bit.VerticalLSB); ok && r == d.Bounds() && src.Bounds() == d.rect && sp.X == 0 && sp.Y == 0 {
		// Exact size, full frame, image4bit encoding: fast path!
		next = img.Pix
	} else {
		// Double buffering.
		if d.next == nil {
			d.next = image4bit.NewVerticalLSB(d.rect)
		}
		next = d.next.Pix
...
//
// This function accepts the content of image4bit.VerticalLSB.Pix.
func (d *Dev) Write(pixels []byte) (int, error) {
	if len(pixels) != len(d.buffer) {
		return 0, fmt.Errorf("ssd1327: invalid pixel stream length; expected %d bytes, got %d bytes", len(d.buffer), len(pixels))
	}
...
func (d *Dev) Scroll(o Orientation, rate FrameRate, startLine, endLine int) error {
	...
	if startLine&1 != 0 || startLine < 0 || startLine >= h {
		return fmt.Errorf("invalid startLine %d", startLine)
	}
	if endLine&1 != 0 || endLine < 0 || endLine > h {
		return fmt.Errorf("invalid endLine %d", endLine)
	}

	startPage := uint8(startLine / 2)
	endPage := uint8(endLine / 2)
	d.scrolled = true
	if o == Left || o == Right {
		// page 28
...
func (d *Dev) Invert(blackOnWhite bool) error {
	// Different command than ssd1306.
	b := []byte{0xA4}
	if blackOnWhite {
		b[0] = 0xA7
	}
...
func newDev(c conn.Conn, o *Opts, usingSPI bool, dc gpio.PinOut) (*Dev, error) {
	w := o.W
	h := o.H
	if w < 8 || w > 128 || w&1 != 0 {
		return nil, fmt.Errorf("ssd1327: invalid width %d", w)
	}
	if h < 8 || h > 128 || h&1 != 0 {
		return nil, fmt.Errorf("ssd1327: invalid height %d", h)
	}
	...
	if err := d.sendCommand(getInitCmd(w, h, o.Rotated)); err != nil {
		return nil, err
	}
	return d, nil
}
...
func getInitCmd(w, h int, rotated bool) []byte {
	...
	*/
	...
	return []byte{
		0xFD, 0x12, // Unlock commands, just in case.
		0xAE,       // Display off
		0xA1, 0x00, // Set display offset; 0
		0xA2, 0x20, // Set offset
		0xA8, uint8(h - 1), // set mux

		//0xA0, 0x51, // Re-map enable column, COM, COM split odd even.
		...
		0x15, 0, uint8(w - 1), // Set column address (Width)
		0x75, 0, uint8(h - 1), // Set page address (Pages)
		0xAF, // Display on
	}
}
...
func (d *Dev) sendCommand(c []byte) error {
	...
	if d.spi {
		if d.dc == nil {
			// 3-wire SPI.
			return errors.New("ssd1327: 3-wire SPI mode is not yet implemented")
		}
		// 4-wire SPI.
