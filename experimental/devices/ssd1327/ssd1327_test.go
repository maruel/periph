...
func TestI2C_Scroll(t *testing.T) {
	...
	if dev.Scroll(Left, FrameRate64, 1, 8) == nil {
		t.Fatal("invalid start")
	}
	if dev.Scroll(Left, FrameRate64, 8, 0) == nil {
		t.Fatal("reversed start and end")
	}
	if dev.Scroll(Left, FrameRate64, 0, 9) == nil {
		t.Fatal("invalid end")
	}
	if err := dev.Scroll(Left, FrameRate64, 0, -1); err != nil {
		t.Fatal(err)
	}
