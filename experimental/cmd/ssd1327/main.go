...
// ssd1327 writes to a display driven by a ssd1327 controler.
package main
...
func findFile(name string) string {
	...
	for _, p := range strings.Split(os.Getenv("GOPATH"), ":") {
		if len(p) != 0 {
			if p2 := filepath.Join(p, "src/periph.io/x/periph/experimental/cmd/ssd1327", name); access(p2) {
				return p2
			}
		}
	}
...
func convert(s *ssd1327.Dev, src image.Image) *image4bit.VerticalLSB {
	screenBounds := s.Bounds()
	size := screenBounds.Size()
	src = resize(src, size)
	img := image4bit.NewVerticalLSB(screenBounds)
	r := src.Bounds()
...
func mainImpl() error {
	...
	o := ssd1327.Opts{W: *w, H: *h, Rotated: *rotated}
	...
	if g != nil {
		...
		imgs := make([]*image4bit.VerticalLSB, len(g.Image))
		for i := range g.Image {
	...
	if src == nil {
		...
		src = image4bit.NewVerticalLSB(s.Bounds())
	}
...
func main() {
	if err := mainImpl(); err != nil {
		fmt.Fprintf(os.Stderr, "ssd1327: %s.\n", err)
		os.Exit(1)
	}
}
