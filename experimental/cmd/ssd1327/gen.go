...
//
// It exists so cmd/ssd1327 does not depend on golang.org/x/image/...
//
...
func mainImpl() error {
	...
	const base = 0x21
	glyphs := [0x80 - base]image4bit.VerticalLSB{}
	for i := range glyphs {
		glyphs[i] = *image4bit.NewVerticalLSB(image.Rect(0, 0, 6, 13))
		drawer := font.Drawer{
			Src:  &image.Uniform{C: image4bit.Gray4(15)},
			Dst:  &glyphs[i],
